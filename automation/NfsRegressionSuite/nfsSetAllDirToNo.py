...
from cbrequest import get_apikey, get_url, umountVolume_new, mountNFS, \
    ...
    get_ntwInterfaceAndIP, sendrequest
...
#2. while Ios are running set all dir to No
#3. verify if IOs are running or not

testcase = 'Set all dir to no while IOs are running'

logging.info('----Start of testcase "%s"----', testcase)

if len(sys.argv) < 2:
    print "Arguments are not correct, Please provide as follows..."
    print "python nfsSetAllDirToNo.py conf.txt vdbench_file_to_run"
        ...
        for nfs in listNfs:
            ...
            if authnetwork == ClientIP:
                ...
                if "errorcode" in data4["UpdateNfsProtocolResponse"]:
                    errormsg = str(data4["UpdateNfsProtocolResponse"]["errortext"])
...
ClientIP = get_ntwInterfaceAndIP(tsmIP)
if ClientIP[0] == 'FAILED':
    endTime = ctime()
    print ClientIP[1]
    logAndresult(testcase, 'BLOCKED', ClientIP[1], startTime, endTime)
else:
    ClientIP = ClientIP[1]
    logging.debug('local client IP : "%s"', ClientIP)
...
vol = {'name': 'allDirNFSVol', 'tsmid': tsmID, \
...
if check_vdbench:
    ...
    if setalldir[0] == 'FAILED':
    	endTime = ctime()
    	logAndresult(testcase, 'FAILED', setalldir[1], startTime, endTime)
    else:
    	logging.info('ALL dir is set to no for Authorized NFS '\
                        'client "%s"', ClientIP)
	
        time.sleep(10)#after setting all dir to NO waiting fo 10s 
        logging.info('After setting all dir to NO , checking whether '\
                'vdbench is running or not')
        check_vdbench = is_vdbench_alive(volname)
        if not check_vdbench:
            logging.debug('Vdbench has stopped running')
            endTime = ctime()
            msg = 'Vdbench has stopped running after seeting all dir to NO'
            logAndresult(testcase, 'FAILED', msg, startTime, endTime)
        logging.debug('After setting all dir to no, vdbench is still running')
else:
    logging.debug('Vdbench has stopped running')
    endTime = ctime()
...
endTime = ctime()
resultCollection('Set all dir to no, testcase is', ['PASSED',' '],\
                        startTime, endTime)
