...
if len(sys.argv) < 5:
    ...
    print bcolors.WARNING + "python ReadonlyExecution.py config.txt <storage_protocol(nfs/cifs/fc/iscsi/all)> <copy/delete/create>(optional) on/off filename_for_copy(optional)" + bcolors.ENDC
    exit()
    ...
    for x in range(1, int(config['Number_of_NFSVolumes'])+1):
        ...
        if output[0] == "PASSED":
            ...
            if delete:
                ...
                print out[0]
                print readonlyvalue
                if readonlyvalue == "on" and out[0] == "FAILED":
                  resultCollection("Delete file \"%s\" operation on volume %s is not possible because read only is ON:" %(filename, config['volDatasetname%d' %(x)]), ("PASSED", ' '), startTime, endTime)
                elif readonlyvalue == "off" and out[0] == "PASSED":
                 resultCollection("Delete file \"%s\" operation on volume %s is possible because read only is OFF:" %(filename, config['volDatasetname%d' %(x)]), ("PASSED", ' '), startTime, endTime)
                else:
                  resultCollection("Readonly feature on volume %s is not working when readonly is \"%s\" :" %(config['volDatasetname%d' %(x)],readonlyvalue), ("FAILED", ' '), startTime, endTime)
                executeCmd('umount  mount/%s' %(config['volMountpoint%d' %(x)]))
        ...
        if output[0] == "PASSED":
            ...
            if mountOutput[0] == "PASSED":
                ...
                if delete:
                    ...
                    endTime = ctime()
                    if readonlyvalue == "on" and out[0] == "FAILED":
                       resultCollection("Delete file \"%s\" operation on volume %s is not possible because read only is ON:" %(filename, config['voliSCSIDatasetname%d' %(x)]), ("PASSED", ' '), startTime, endTime)
                    elif readonlyvalue == "off" and out[0] == "PASSED":
                       resultCollection("Delete file \"%s\" operation on volume %s is possible because read only is OFF:" %(filename, config['voliSCSIDatasetname%d' %(x)]), ("PASSED", ' '), startTime, endTime)
                    else:
                       resultCollection("Readonly feature on volume %s is not working when readonly is \"%s\" :" %(config['volDatasetname%d' %(x)],readonlyvalue), ("FAILED", ' '), startTime, endTime)
                ...
                endTime = ctime()
                resultCollection("Mount of iSCSI Volume %s failed" %(config['voliSCSIDatasetname%d' %(x)]), mountOutput, startTime, endTime)

            ### Logout to ISCSI
            output=executeCmd('iscsiadm -m node --targetname "%s" --portal "%s:3260" --logout | grep Logout' %(iqnname[0].strip(), config['voliSCSIIPAddress%d' %(x)]))
            if output == "FAILED":
                endTime = ctime()
                resultCollection("Logout of iSCSI Volume %s failed" %(config['voliSCSIDatasetname%d' %(x)]), output, startTime, endTime)
        else:
            endTime = ctime()
    ...
    for x in range(1, int(config['Number_of_CIFSVolumes'])+1):
        ...
        if output[0] == "PASSED":
           ...
           if copy:
                ...
                executeCmd('rm -r mount/%s/*'%(config['volCifsMountpoint%d' %(x)]))
           ...
           if delete:
                ...
                print out[0]
                if readonlyvalue == "on" and out[0] == "FAILED":
                  resultCollection("Delete file \"%s\" operation on volume %s is not possible because read only is ON:" %(filename, config['volCifsDatasetname%d' %(x)]), ("PASSED", ' '), startTime, endTime)
                elif readonlyvalue == "off" and out[0] == "PASSED":
                  resultCollection("Delete file \"%s\" operation on volume %s is possible because read only is OFF:" %(filename, config['volCifsDatasetname%d' %(x)]), ("PASSED", ' '), startTime, endTime)
                else:
                  resultCollection("Readonly feature on volume %s is not working when readonly is \"%s\" :" %(config['volCifsDatasetname%d' %(x)],readonlyvalue), ("FAILED", ' '), startTime, endTime)
                  executeCmd('umount  mount/%s' %(config['volCifsMountpoint%d' %(x)]))
        else:
            endTime = ctime()
