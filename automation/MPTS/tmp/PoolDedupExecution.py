...
if  nfsFlag == 1 or allFlag == 1:
    ...
    for x in range(1, int(config['Number_of_NFSVolumes'])+1):
             ...
             executeCmd('umount -rf mount/%s'%(config['volMountpoint%d' %(x)]))
             executeCmd('rm -rf mount/%s'%(config['volMountpoint%d' %(x)]))
        ...
        ######  Mount and copy NFS ###############
             ...
             if output[0] == "PASSED":
                 print "mount passed"
                 #executeCmd('cp -v %s mount/%s/%s' %(testfile, config['volMountpoint%d' %(x)],testfile))
                 executeCmd('rm -rf mount/%s'%(config['volMountpoint%d' %(x)]))
                 ...
                 print dr1
                 for y in range (1, 3):
                   executeCmd("cp -v %s mount/%s/%s"%(testfile,config['volMountpoint%d' %(x)], testfile +'%d' %(y)))
                   time.sleep(10)
                 print "sleep for 30 sec"
                 time.sleep(60)
             ...
             if  output[0] == "PASSED":
                 if  dedupvalue == "on" and dr2 > dr1 and float(volused2) > float(volused1):
                     ...
                     resultCollection("Dedup Feature is not working on\'%s\' using %s when dedup is \'%s\' " %(PoolName,config['volDatasetname%d' %(x)],dedupvalue),["FAILED", ' '],startTime, endTime)
...
if  cifsFlag ==1 or allFlag ==1:
    for x in range(1, int(config['Number_of_CIFSVolumes'])+1):
        ...
        ###### Mount and Copy CIFS #################
             ...
             if  output[0]=="PASSED":
                 #executeCmd('cp -v %s mount/%s/%s' %(testfile, config['volCifsMountpoint%d' %(x)],testfile))
                 executeCmd('rm -rf mount/%s'%(config['volCifsMountpoint%d' %(x)]))
                 executeCmd('rm -r mount/%s/.recycle/'%(config['volCifsMountpoint%d' %(x)]))
                 print "sleep for 30 sec"
                 time.sleep(60)
                 ...
                 for y in range (1, 3):
                     executeCmd("cp -v %s mount/%s/%s" %(testfile,config['volCifsMountpoint%d' %(x)], testfile +'%d' %(y)))
                     time.sleep(10)
                 print "sleep for 30 sec"
                 time.sleep(60)
                 dedupRatio2 = getControllerInfo(ip, passwd,"zpool list %s | awk '{print $6}' | tail -n1" %(PoolName),"output.txt")
             ...
             if  output[0] == "PASSED":
                   if  dedupvalue == "on" and dr2 > dr1 and float(volused2) > float(volused1):
                      ...
                      resultCollection("Dedup Feature is not working on\'%s\' using %s when dedup is \'%s\' " %(PoolName,config['volCifsDatasetname%d' %(x)],dedupvalue),["FAILED", ' '],startTime, endTime)
             else:
    ...
    for x in range(1, int(config['Number_of_ISCSIVolumes'])+1):
             ...
             ### mkfs
                 ...
                 if mountOutput[0] == "PASSED":
                   ...
                   print"sleep for 30 sec"
                   time.sleep(300)
                   #executeCmd('cp -v %s mount/%s/%s' %(testfile, config['voliSCSIMountpoint%d' %(x)],sys.argv[3]))
                   dedupRatio1 = getControllerInfo(ip, passwd,"zpool list %s | awk '{print $6}' | tail -n1" %(PoolName),"output.txt")
                   print dedupRatio1
                   dr1=float(dedupRatio1[0:-2])
                   ...
                   print volused1
                   for y in range (1, 3):
                     executeCmd("cp -v %s mount/%s/%s" %(testfile,config['voliSCSIMountpoint%d' %(x)], testfile +'%d' %(y)))
                     time.sleep(5)
                   print "sleep for 30 sec"
                   time.sleep(150)
                   dedupRatio2 = getControllerInfo(ip, passwd,"zpool list %s | awk '{print $6}' | tail -n1" %(PoolName),"output.txt")
                 ...
                 if  mountOutput[0] == "PASSED":
                    if  dedupvalue == "on" and dr2 > dr1 and float(volused2) > float(volused1):
                      ...
                      resultCollection("Dedup Feature is not working on\'%s\' using %s when Dedup is \'%s\' "%(PoolName,config['voliSCSIDatasetname%d' %(x)],dedupvalue),["FAILED", ' '],startTime, endTime)
