...
if len(sys.argv) < 5:
    ...
    print "python QosDatasetDelete.py config.txt ISCSI/NFS/CIFS/FC/ALL nodeip password "
    exit()
...
if sys.argv[2].lower() == "%s" %("nfs"):
    ...
    print "Argument is not correct.. Correct way as below"
    print "python QosDatasetDelete.py dedup.txt NFS/CIFS/ISCSI/FC nodeip password "
...
def poolfunction(fullpoolname):
                 ...
                 #print pooluuid
                 poolrengcmd1 =' reng list | grep -A 9 %s | grep \'Renegade IO remainder\' | awk \'{print$4}\' ' %(fullpoolname)
                 print poolrengcmd1
                 global poolrengiops
                 poolrengiops= getControllerInfo(nodeip, nodepasswd,poolrengcmd1,"output.txt")
                 print "poolrengremainediops= "+ poolrengiops
                 poolrengcmd2 = " reng list | grep -A 9 %s | grep 'Renegade throughput remainder' | awk '{print$4}'" %(fullpoolname)
                 print poolrengcmd2
        ...
        for x in range(1, int(config['Number_of_NFSVolumes'])+1):
            if filesystem_name == "%s" %(config['volDatasetname%d' %(x)]):
                ...
                if iopscontrolled == "true" and throughputcontrolled == "false":
                  ...
                  job_id = response["deleteFileSystemResponse"]["jobid"]
