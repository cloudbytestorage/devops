...
#iscsi
     for x in range(1, int(config['Number_of_ISCSIVolumes'])+1):
       if filesystem_name == "%s" %(config['voliSCSIDatasetname%d' %(x)]):
          id= filesystem_id
          #print "hi"
          querycommand='command=updateFileSystem&id=%s&quotasize=%s&noofcopies=%s&readonly=%s' %(id, config['voliSCSIQuotasize%d' %(x)],config['voliSCSINoofCopies%d' %(x)], config['volReadOnly%d' %(x)])
