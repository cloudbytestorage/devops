                ...
                if filesystem['name'] == cloneName:
                    ...
                    if filesystem_id is not None:
                        ...
                        if out[0] == "PASSED":
                            ...
                            executeCmd('umount  %s:/%s' %(config['volIPAddress%d' %(x)], cloneName))
                        else:
                            endTime = ctime()
                    ...
                    if filesystem_id is not None:
                        ...
                        if output[0] == "PASSED":
                            ...
                            if mountOutput[0] == "PASSED":
                                print "Clone dataset %s mounted successfully" %(cloneName)
                                executeCmd('md5sum mount/%s/%s | awk \'{print $1}\' > mount/%s_md5sum_mount' %(cloneName, filename, filename))
                                diffoutput=executeCmd('diff mount/%s_md5sum mount/%s_md5sum_mount' %(filename, filename))
                                endTime = ctime()
                            ...
                            if output == "FAILED":
                                endTime = ctime()
                                resultCollection("Logout of iSCSI Volume %s" %(cloneName), output, startTime, endTime)
        else:
            endTime = ctime()
