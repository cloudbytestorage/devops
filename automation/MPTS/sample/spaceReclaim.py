...
if iscsiFlag == 1 or allFlag == 1:
    for x in range(1, int(config['Number_of_ISCSIVolumes'])+1):
        ...
        ######Mount
        if output[0] == "PASSED":
            executeCmd('fdisk /dev/sdc < fdisk_response_file' )
            executeCmd('mkfs.ext3 /dev/sdc1')
            mountOutput=executeCmd('mount /dev/sdc1  mount/%s' %(config['voliSCSIMountpoint%d' %(x)]))
            #####Copy
            if mountOutput[0] == "PASSED":
                print "mount successfull"
                q=getoutput('df -h | grep /dev/sdc1 | awk \'{print $2}\' | sed s/\[A-Z\]//')
                print " used space of volume on client = %s after mount" % q[1].strip()
                ...
                q1=getoutput('df -h | grep /dev/sdc1 | awk \'{print $2}\' | sed s/\[A-Z\]//')
                print " used space of volume on client = %s after copying file" %q1[1].strip()
                ...
                time.sleep(5)
                q2=getoutput('df -h | grep /dev/sdc1 | awk \'{print $2}\' | sed s/\[A-Z\]//')
                print " used space of volume on client = %s after deleting the volume" %q2[1].strip()
