...
if [ $? -eq 0 ]
then
    failed=`grep -i "FAIL" results/result.csv | wc -l`
    passed=`grep -i "PASS" results/result.csv | wc -l`
    total=`expr $failed + $passed`
    status=PASSED
    ...
    then
        ...
        grep -i fail results/result.csv >> mail.txt
        status=FAILED
    fi
    ...
    #mail -s "Automation Report -- `sed -n 1p installed_version` -- $status" -a results/result.csv -r cbauto@cloudbyte.com  -c ajesh.baby@cloudbyte.com, cb-qa@cloudbyte.com < mail.txt
    #mail -s "Automation Report -- `sed -n 1p installed_version` -- $status" -a results/result.csv -r cbauto@cloudbyte.com  -c ajesh.baby@cloudbyte.com, rohit.mishra@cloudbyte.com, apeksha.khakharia, mardan.singh@cloudbyte.com < mail.txt
    mail -s "Automation Report -- `sed -n 1p installed_version` -- $status" -a results/result.csv -r cbauto@cloaaudbyte.com  -c ajesh.baby@cloudbyte.com, rohit.mishra@cloudbyte.com, apeksha.khakharia@cloudbyte.com, mardan.singh@cloudbyte.com < mail.txt
    #mail -s "Automation Report -- `sed -n 1p installed_version` -- $status" -a results/result.csv -r cbauto@cloudbyte.com rameshbabu.kl@cloudbyte.com < mail.txt
else
    ...
    cat mail.txt.base >> mail.txt
    mail -s "Automation Report -- `sed -n 1p installed_version`" -r cbauto@cloudbyte.com ajesh.baby@cloudbyte.com -c rameshbabu.kl@cloudbyte.com < mail.txt
fi
