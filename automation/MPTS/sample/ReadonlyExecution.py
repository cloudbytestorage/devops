            ...
            if delete:
                ...
                if readonlyvalue == "on" and out[0] == "FAILED":
                 ...
                 resultCollection("Delete file \"%s\" operation on volume %s is possible because read only is off:" %(filename, config['volDatasetname%d' %(x)]), ("PASSED" ' '), startTime, endTime)
                  ...
                  resultCollection("Readonly feature on volume %s is not working:" %(config['volDatasetname%d' %(x)]), ("FAILED" ' '), startTime, endTime)
                executeCmd('umount  %s:/%s' %(config['volIPAddress%d' %(x)],config['volMountpoint%d' %(x)]))
            ...
            if delete:
               ...
               # out2 = executeCmdNegative('ls mount/%s | grep %s' %(config['volCifsMountpoint%d' %(x)], filename))
                ...
                if readonlyvalue == "on" and out[0] == "FAILED":
                  ...
                  resultCollection("Readonly feature on volume %s is not working:" %(config['volCifsDatasetname%d' %(x)]), ("FAILED" ' '), startTime, endTime)
                  executeCmd('umount  %s:/%s' %(config['volIPAddress%d' %(x)],config['volMountpoint%d' %(x)]))
        else:
            endTime = ctime()
