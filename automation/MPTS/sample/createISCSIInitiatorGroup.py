    ...
    for account in accounts:
        if account['name']  == "%s" %(config['accountName%d' %(x)]):
            ...
            if not "errortext" in str(data):
                resultCollection("sucessfully created iscsi initiator group in %s" %account['name'], ["PASSED"," "], startTime, endTime)
            else:
                errorstatus= str(data['tsmiSCSIInitiatorResponse']['errortext'])
                resultCollection("failed to created iscsi initiator group in %s" %account['name'], ["FAILED",errorstatus], startTime, endTime)        
