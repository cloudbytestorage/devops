...
for x in range(1, int(config['Number_of_TSMs'])+1):
  ...
  for auth_group in auth_groups :
      ...
      if AUTH_Name1 == auth_name:
          break ;
  net_bios_name = '%s%s' %(Acc_name, tsm_name1)
  querycommand = 'command=updateTsmCifsService&netbiosname=%s&description=&authentication=user&workgroup=workgroup&doscharset=CP437&unixcharset=UTF-8&loglevel=Minimum&timeserver=false&authgroupid=%s&id=%s' %(net_bios_name, auth_id, cifs_id)
