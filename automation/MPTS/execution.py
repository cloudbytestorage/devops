...
if nfsFlag == 1 or allFlag == 1:
    ...
    for x in range(1, int(config['Number_of_NFSVolumes'])+1):
        ...
        ### Mount
        executeCmd('mount -o nolock -t nfs %s:/%s mount/%s' %(config['volIPAddress%d' %(x)], config['volMountpoint%d' %(x)], config['volMountpoint%d' %(x)]))
...
if iscsiFlag == 1 or allFlag == 1:
    #executeCmd('iscsiadm -m session -u')
    for x in range(1, int(config['Number_of_ISCSIVolumes'])+1):
        startTime = ctime()
        ...
        if output[0] == "PASSED":
            ...
            if output == "FAILED":
                ...
                print "Logout of iSCSI Volume %s FAILED" %(config['voliSCSIDatasetname%d' %(x)])
        else:
            endTime = ctime()
