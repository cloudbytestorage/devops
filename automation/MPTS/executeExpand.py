...
if nfsFlag == 1 or allFlag ==1:
    for x in range(1, int(config['Number_of_NFSVolumes'])+1):
        ...
        if output[0] == "PASSED":
            ...
            endTime = ctime()
            resultCollection("Further Execution of TCs Skipped on NFS Volume %s" %(config['volDatasetname%d' %(x)]), ('FAILED','Due to Mount Failed'),startTime,endTime) 
...
if iscsiFlag == 1 or allFlag == 1:
    for x in range(1, int(config['Number_of_ISCSIVolumes'])+1):
        ...
        if output[0] == "PASSED":
            ...
            #print device2
            #device2 = getDiskAllocatedToISCSI(config)
            #print device2_2
            #print device2_2['%s' %config['voliSCSIDatasetname%d' %(x)]]
            #print 'Kaushik'
            #exit()
            executeCmd('fdisk /dev/%s < fdisk_response_file2' %(device2))
            time.sleep(5)
            quota=getoutput('fdisk -l | grep /dev/%s: |  awk {\'print $5\'}' %(device2))
            q= int(quota[0])/(1024*1024*1024)
            print q

            mkfs_result = executeCmd('mkfs.ext3 /dev/%s2' %(device2))
            if mkfs_result[0] == "FAILED":
                endTime = ctime()
                resultCollection("Not able to write filesyatem on newly extended partion on iscsi Volume %s" %(config['voliSCSIDatasetname%d' %(x)]), ['FAILED', ''], startTime, endTime)
                continue
            mountOutput1=executeCmd('mount /dev/%s2  expand/' %(device2))
            mountOutput=executeCmd('mount /dev/%s1  mount/%s' %(device2, config['voliSCSIMountpoint%d' %(x)]))
            print mountOutput[0]
            #####Copy
            if mountOutput[0] == "PASSED" and mountOutput1[0] == "PASSED":
                ...
                if output[0] == "FAILED":
                    ...
                    continue
                #q2=getoutput('df -h | grep /dev/%s2 | awk \'{print $2}\' | sed s/G//' %(device2))
                #print float(q2[0])
                ...
                print "mount failed"
                endTime = ctime()
                resultCollection("Mount of expented iSCSI Volume %s Failed" %(config['voliSCSIDatasetname%d' %(x)]), ['FAILED', ''], startTime, endTime)
                                                                                                                                                                                    ######Logout to ISCSI
            output=executeCmd('iscsiadm -m node --targetname "%s" --portal "%s:3260" --logout | grep Logout' %(iqnname[0].strip(), config['voliSCSIIPAddress%d' %(x)]))
            if output == "FAILED":
                endTime = ctime()
