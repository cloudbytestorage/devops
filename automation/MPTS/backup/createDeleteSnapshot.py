...
if len(sys.argv) < 6:
    print bcolors.WARNING + "python createDeleteSnapshot.py config.txt <storage_protocol(nfs/cifs/fc/iscsi/all or tsm)> create/delete snapshot_name Node_IP" + bcolors.ENDC
    exit()
                ...
                if tsm_dataset_id is not None:
                    ...
                    if createSnp:
                        ...
                        if 'errortext' in str(tsmSnpResp):
                            ...
                            getControllerInfo(IP, 'test', 'zfs list -t snapshot | grep %s' %(Name), "tsm_snapshot_result.txt")
                    ...
                    if filesystem_id is not None:
                        ...
                        if createSnp:
                            ...
                            if 'errortext' in str(createSnpResp):
                                ...
                                endTime = ctime()
                                getControllerInfo(IP, 'test', 'zfs list -t snapshot | grep %s' %(Name), "snapshot_result.txt")
                        ...
                        if deleteSnp:
                            ...
                            if 'errortext' in str(data3):
                                ...
                                if hasClone == -1:
                                    ...
                                    resultCollection("Snapshot \"%s\" on volume %s has a clone : " %(Name, config['volCifsDatasetname%d' %(x)]), ['PASSED', errorstatus], startTime, endTime)
                            else:
                                endTime = ctime()
                                cmd = 'zfs list -t snapshot | grep %s' %(Name)
                                getControllerInfo(IP, 'test', cmd, "snapshot_result.txt")
                        ...
                        if deleteSnp:
                            ...
                            if 'errortext' in str(data3):
                                ...
                                if hasClone == -1:
                                    ...
                                    resultCollection("Snapshot \"%s\" on volume %s has a clone : " %(Name, config['volDatasetname%d' %(x)]), ['PASSED', errorstatus], startTime, endTime)
                            else:
                                endTime = ctime()
                                cmd = 'zfs list -t snapshot | grep %s' %(Name)
                                getControllerInfo(IP, 'test', cmd, "snapshot_result.txt")
                        ...
                        if deleteSnp:
                            ...
                            if 'errortext' in str(data3):
                                ...
                                if hasClone == -1:
                                    ...
                                    resultCollection("Snapshot \"%s\" on volume %s has a clone : " %(Name, config['voliSCSIDatasetname%d' %(x)]), ['PASSED', errorstatus], startTime, endTime)
                            else:
                                endTime = ctime()
                                cmd = 'zfs list -t snapshot | grep %s' %(Name)
                                getControllerInfo(IP, 'test', cmd, "snapshot_result.txt")
                                out = executeCmdNegative('cat snapshot_result.txt | grep %s > snapshot_result2.txt' %(Name))
