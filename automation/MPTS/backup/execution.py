...
if iscsiFlag == 1 or allFlag == 1:
    for x in range(1, int(config['Number_of_ISCSIVolumes'])+1):
        ...
        if output[0] == "PASSED":
            ### mkfs
            if write_mkfs:
                executeCmd('fdisk /dev/sde < fdisk_response_file' )
                quota=getoutput('fdisk -l | grep /dev/sde: |  awk {\'print $5\'}')
                q= int(quota[0])/(1024*1024*1024)
                # print q
                executeCmd('mkfs.ext3 /dev/sde1')

            ### mount
            # executeCmd('mkdir -p mount/%s' %(config['voliSCSIMountpoint%d' %(x)]))
            mountOutput=executeCmd('mount /dev/sde1  mount/%s' %(config['voliSCSIMountpoint%d' %(x)]))
            ...
            if mountOutput[0] == "PASSED":
                ...
                if copy:
                    ...
                    q1=getoutput('df -h | grep /dev/sde1 | awk \'{print $2}\' | sed s/G//')
                    # print q1[0]
