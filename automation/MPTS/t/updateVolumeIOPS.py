...
for filesystem in filesystems:
     ...
     for x in range(1, int(config['Number_of_NFSVolumes'])+1):
       if filesystem_name == "%s" %(config['volDatasetname%d' %(x)]):
           id = filesystem_id
           querycommand='command=updateQosGroup&id=%s&iops=%s&graceallowed=%s' %(id, config['volIops%d' %(x)], config['volGraceallowed%d' %(x)])
...
#cifs
     for x in range(1, int(config['Number_of_CIFSVolumes'])+1):
       if filesystem_name == "%s" %(config['volCifsDatasetname%d' %(x)]):
           id = filesystem_id
           querycommand='command=updateQosGroup&id=%s&iops=%s&graceallowed=%s' %(id, config['volCifsIops%d' %(x)], config['volCifsGraceallowed%d' %(x)])
...
#iscsi
     for x in range(1, int(config['Number_of_ISCSIVolumes'])+1):
       if filesystem_name == "%s" %(config['voliSCSIDatasetname%d' %(x)]):
           id = filesystem_id
           querycommand='command=updateQosGroup&id=%s&iops=%s&graceallowed=%s' %(id, config['voliSCSIIops%d' %(x)], config['voliSCSIGraceallowed%d' %(x)])
  ...
  #fc
     for x in range(1, int(config['Number_of_fcVolumes'])+1):
       if filesystem_name == "%s" %(config['volfcDatasetname%d' %(x)]):
           id = filesystem_id
           querycommand='command=updateQosGroup&id=%s&iops=%s&graceallowed=%s' %(id, config['volfcIops%d' %(x)], config['volfcGraceallowed%d' %(x)])
