...
if len(sys.argv) < 2:
    ...
    print " python createAuthgrp.py config.txt AuthGroup-name(optional)"
    exit()
if len(sys.argv)==2:
    AuthGrp="AuthGrp"
elif len(sys.argv)==3:
    ...
    for account in accounts:
        if account['name']  == "%s" %(config['accountName%d' %(x)]):
            ...
            if not "errortext" in str(data):
                resultCollection("sucessfully created iscsi authentication group \'%s\' in %s" %(authgrp_name,account['name']), ["PASSED"," "], startTime, endTime)
            else:
                errorstatus= str(data['tsmiSCSIAuthGroupResponse']['errortext'])
                resultCollection("failed to created iscsi authentication group \'%s\' in %s" %(authgrp_name,account['name']), ["FAILED",errorstatus], startTime, endTime)
