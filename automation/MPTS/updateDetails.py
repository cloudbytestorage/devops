...
### using md5sum of password for 1.4.0 build
#payload = {'command': 'login', 'username': 'admin', 'password': md5_org_pwd, 'domain': '/', 'response': 'json'}

### using direct text without converting it to md5sum, for build 1.4.1 onword
payload = {'command': 'login', 'username': 'admin', 'password':config['admin_org_password'], 'domain': '/', 'response': 'json'}
...
print user_id
### Update User ID
querystring = 'command=updateUser&id=%s&password=%s&oldpassword=%s&firstname=%s&lastname=%s&email=%s&mobileno=%s' %(user_id, md5_set_pwd,md5_org_pwd, config['firstname'], config['lastname'], config['email'], config['mobile'])
r = s.get(stdurl_noapikey+querystring, verify=False)
