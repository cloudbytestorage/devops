...
for x in range(1, int(config['Number_of_ISCSIVolumes'])+1):
    ...
    if negativeFlag == 1:
        ...
        if iqnname==[]: 
            print "Negative testcase-iscsi volume %s login failed on the client with dummy Mutualchap credentials, testcase passed" %(config['voliSCSIDatasetname%d' %(x)])
            endTime = ctime()
            resultCollection("Negative testcase-iscsi volume %s login failed on the client with dummy Mutualchap credentials" %(config['voliSCSIDatasetname%d' %(x)]),["PASSED",""], startTime, endTime)
        ### some iscsi volumes discovered
            ...
            if output[0] == "PASSED":
                print "Negative testcase-iscsi volume %s login passed on the client with dummy Mutualchap credentials, test case failed" %(config['voliSCSIDatasetname%d' %(x)])
                endTime = ctime()
                resultCollection("Negative testcase-iscsi volume %s login passed on the client with dummy Mutualchap credentials" %(config['voliSCSIDatasetname%d' %(x)]),["FAILED",""], startTime, endTime)
    ...
    # for positive testcase
        ...
        ###no iscsi volumes discovered
        if iqnname==[]:
            print "iscsi volume %s login failed on the client with Mutualchap credentials" %(config['voliSCSIDatasetname%d' %(x)])
            endTime = ctime()
            resultCollection("iscsi volume %s login failed on the client with Mutualchap credentials" %(config['voliSCSIDatasetname%d' %(x)]),["FAILED",""], startTime, endTime)
        ### some iscsi volumes discovered
        else:
            output=executeCmd('iscsiadm -m node --targetname "%s" --portal "%s:3260" --login | grep Login' %(iqnname[0].strip(), config['voliSCSIIPAddress%d' %(x)]))
            ### iscsi volume login successfull
            if output[0] == "PASSED":
                print "iscsi volume %s login passed on the client with Mutualchap credentials" %(config['voliSCSIDatasetname%d' %(x)])
                endTime = ctime()
                resultCollection("iscsi volume %s login passed on the client with Mutualchap credentials" %(config['voliSCSIDatasetname%d' %(x)]),["PASSED",""], startTime, endTime)
                ...
                if output[0] == "PASSED":
                    endtime = ctime() 
                    resultCollection("Creation of File on ISCSI Volume %s passed on the client with Mutualchap credentials" %(config['voliSCSIDatasetname%d' %(x)]),["PASSED",""], startTime, endTime)
                else:
                    endtime = ctime()
                    resultCollection("Creation of File on ISCSI Volume %s passed on the client with Mutualchap credentials" %(config['voliSCSIDatasetname%d' %(x)]),["FAILED",""], startTime, endTime)
            ### iscsi volume login unsuccessfull
            else:
                print "iscsi volume %s login failed on the client with Mutualchap credentials" %(config['voliSCSIDatasetname%d' %(x)])
                endTime = ctime()
