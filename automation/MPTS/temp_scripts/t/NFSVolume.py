...
#for x in range (1, NoofNFSVolumes+1):
    ...
    ###Stage3 Manage NFS Serveice
    querycommand = 'command=manageNfs&id=%s&managedstate=%s' %(storage_id, config['volManagedState%d' %(x)])
    resp_nfsService = sendrequest(stdurl, querycommand)
    filesave("logs/ManageNFSService.txt", "w", resp_nfsService)
    ...
    querycommand ='command=addFsCifsService&datasetid=%s&name=%s&description=default&status=false&readonly=false&browseable=true&inheritpermissions=true&recyclebin=true&hidedotfiles=true&name=null' %(storage_id, config['volDatasetname%d' %(x)])
