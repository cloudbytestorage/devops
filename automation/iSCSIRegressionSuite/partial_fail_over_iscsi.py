...
logging.info('------------------------------partial fail over test case(iSCSI)'\
        'started------------------------------')
startTime = ctime()
EXECUTE_SYNTAX = 'python partial_fail_over_iscsi.py conf.txt'
FOOTER_MSG = 'partial fail over test case(iSCSI) completed'
BLOCKED_MSG = 'Partial fail over test case(iSCSI) is blocked'
...
def getTsmInfo(tsms):
    ...
    logging.error('partial fail over test case(iSCSI) blocked '\
            'due to %s', tsms[1])
...
def verify_create_volume(result):
    if result[0] == 'PASSED':
        return
    logging.error('partial fail over test case(iSCSI) is blocked Volume '\
            'creation failed')
    is_blocked(startTime, FOOTER_MSG, BLOCKED_MSG)
...
def get_vol_id(volumes, vol_name):
    ...
    if volid is None or accountid is None or voliqn is None or mntpoint is None:
        logging.error('partial fail over test case(iSCSI) is blocked getting '\
...
def verify_iqn(iqn):
    if iqn[0] == 'PASSED':
        return iqn[1]
    logging.debug('partial fail over test case(iSCSI) is blocked '\
            'getting iqn is None')
    is_blocked(startTime, FOOTER_MSG, BLOCKED_MSG)

def verify_add_auth_gp(add_auth_group, auth_gp):
    if add_auth_group[0] == 'FAILED':
        logging.debug('partial fail over test case(iSCSI) is blocked Not '\
...
def verify_iscsi_operation(result, vol_name, action):
    ...
    if 'already exists' in str(result[1]):
        ...
        return
    logging.error('partial fail over test case(iSCSI) is blocked Not able '\
            'to %s %s, Error: %s', action, vol_name, str(result[1]))
    is_blocked(startTime, FOOTER_MSG, BLOCKED_MSG)
...
def verify_IOPS_afterHA(NODE2_IP, PASSWD):
    ...
    if io_output[0] == 'FAILED':
        ...
        logging.debug('Compared result: %s, Error: %s', io_output[0], msg)
        logging.debug('IOPS Error: partial fail over test case(iSCSI) failed...')
        resultCollection('partial fail over test case(iSCSI) failed', \
                ['FAILED', ''], startTime, endTime)
    elif io_output[0] == 'PASSED':
        msg =  "IOPS are  running fine after partial fail over"
        logging.debug('Compared result: %s', msg)
        resultCollection('partial fail over test case(iSCSI) passed', \
...
volumeDict = {'name': 'partialiSCSI1', 'tsmid': tsm_id, 'datasetid': \
...
verify_IOPS_afterHA(NODE2_IP, PASSWD)
time.sleep(170)
logging.debug('partial fail over test case(iSCSI) completed, removing configuration')
