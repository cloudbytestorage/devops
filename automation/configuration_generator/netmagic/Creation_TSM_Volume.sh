rm logs/*
>temp/tsm.txt; >temp/nfsvolume.txt; >temp/iscsivolume.txt; >temp/cifsvolume.txt; >temp/fcvolume.txt; >config.txt; >logs/tracktime;
>tsmlist
...
do
    ...
    read tsmconfirmation
        ...
        echo ""
        read interface
        echo -en Enter the"\033[32m DNS Server IP \033[0m"you have to associate the TSM
        echo ""
        read dnsserver
        echo -en Enter the"\033[32m Router IP \033[0m"you have to associate the TSM
        echo ""
        read router
        echo -en Enter the"\033[32m Subnet \033[0m"you have to associate the TSM
        echo ""
        read subnet
    ...
    then
        ...
        echo "$ipaddress" >> tsmlist
        sed -i s/INTERFACE/$interface/g temp/tsm.txt
        sed -i s/DNSSERVER/$dnsserver/g temp/tsm.txt
        sed -i s/ROUTER/$router/g temp/tsm.txt
        sed -i s/SUBNET/$subnet/g temp/tsm.txt
    else
...
sed -i s/NoofTSMS/$NoofTSMs/g temp/createvml.sh
sed -i s/NoofISCSIVolumes/$NoofISCSIVolumes/g temp/iscsivolume.txt
sed -i s/NoofISCSIVolumes/$NoofISCSIVolumes/g temp/createvml.sh
