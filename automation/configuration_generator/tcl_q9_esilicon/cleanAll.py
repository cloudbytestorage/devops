...
if len(sys.argv) < 2:
    ...
    print "python delete.py NFS"
    print "python delete.py ALL"
    print "python delete.py NFS CIFS FC TSM"
    exit()
for x in range(1, len(sys.argv)):
    if sys.argv[x].lower() == "%s" %("nfs"):
        ...
        print "python delete.py config.txt NFS CIFS FC TSM"
        exit()
...
if iscsiFlag == 1 or allFlag == 1:
    ...
    for filesystem in filesystems:
        ...
        filesystem_name = filesystem['name']
        status = filesystem['status']
        is_iscsi = filesystem['iscsienabled']
        if str(is_iscsi).lower() == 'true':
            if status.lower() == 'online':
                querycommand = 'command=deleteFileSystem&id=%s' %(filesystem_id)
            else:
                querycommand = 'command=deleteFileSystem&id=%s&forcedelete=true' %(filesystem_id)
...
if fcFlag == 1 or allFlag == 1:
    ...
    for filesystem in filesystems:
        ...
        filesystem_name = filesystem['name']
        status = filesystem['status']
        is_fc = filesystem['fcenabled']
        if str(is_fc).lower() == 'true':
            if status.lower() == 'online':
                querycommand = 'command=deleteFileSystem&id=%s' %(filesystem_id)
            else:
                querycommand = 'command=deleteFileSystem&id=%s&forcedelete=true' %(filesystem_id)
    ...
    for pool in pools:
        ...
        for x in range(1, int(config['Number_of_Pools'])+1):
               ...
               if "success" in data["deleteHAPoolResponse"]:
                   ...
                   print "Error in Creating %s : %s " %(pool_name,data["deleteHAPoolResponse"]["errortext"])
               else:
...
if haFlag == 1 or allFlag ==1:
    ...
    for hacluster in haclusters:
         ...
         if "errortext" in data["deleteClusterResponse"]:
             print "Error in deleteing the %s : %s"  %(hacluster_name,data["deleteHAClusterResponse"]["errortext"])
         else:
