...
logging.info('------------------------------gracefull HA test case(NFS)'\
        'started------------------------------')
startTime = ctime()
EXECUTE_SYNTAX = 'python graceFullHANFS.py conf.txt'
FOOTER_MSG = 'gracefull HA test case(NFS) completed'
BLOCKED_MSG = 'Gracefull HA test case(NFS) is blocked'
...
def getTsmInfo(tsms):
    ...
    logging.error('gracefull HA test case(NFS) blocked '\
            'due to %s', tsms[1])
    is_blocked(startTime, FOOTER_MSG, BLOCKED_MSG)
...
def verify_create_volume(result):
    if result[0] == 'PASSED':
        return
    logging.error('gracefull HA test case(NFS) is blocked Volume '\
            'creation failed')
    is_blocked(startTime, FOOTER_MSG, BLOCKED_MSG)
...
def get_vol_id(volumes, vol_name):
    ...
    if volid is None or accountid is None or mntpoint is None:
        logging.error('gracefull HA test case(NFS) is blocked getting volid:'\
...
def verify_IOPS_afterHA(NODE2_IP, PASSWD):
    ...
    if io_output[0] == 'FAILED':
        ...
        logging.debug('Compared result: %s, Error: %s', io_output[0], msg)
        logging.debug('IOPS Error: gracefull HA test case(NFS) failed...')
        resultCollection('Gracefull HA test case(NFS) failed', \
                ['FAILED', ''], startTime, endTime)
    elif io_output[0] == 'PASSED':
        msg =  "IOPS are  running fine after gracefull HA"
        logging.debug('Compared result: %s', msg)
        resultCollection('Gracefull HA test case(NFS) passed', \
...
volumeDict = {'name': 'grceHANFS1', 'tsmid': tsm_id, 'datasetid': dataset_id, \
