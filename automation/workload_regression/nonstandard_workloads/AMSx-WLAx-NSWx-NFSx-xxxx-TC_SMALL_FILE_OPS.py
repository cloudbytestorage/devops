...
#                                                                                                             #
# Testcase Description : This test performs the following fileOps :                                           #
...
# Initialization for Logging file & location 
tcName = sys.argv[0]
...
pool_info = get_pool_info(pool_list, None) #Upon none, func Will take existing single pool
...
account_id = get_account_id(STDURL, None) # 2nd param is acc_name, if None, taken as 1st existing acc
...
# Provide variables for VSM creation (from conf & hardcode -quota as 1T ) in dict format 
...
# Provide variables for Volume creation (from conf & hardcode - quota 500G) in dict format 
...
# Get the info for the Volume created above (this list method response contains id, etc..)
...
# Run the File Create Read ModifyWrite Delete Ops on the NFS mount point 
