'''
###############################################################################################################
# Testcase Name : Multiple_istgtcontrol_refresh_During_IO                                                             #
#                                                                                                             #
...
from utils import check_mendatory_arguments, is_blocked, get_logger_footer, \
        ...
        discover_iscsi_lun, assign_iniator_gp_to_LUN
...
logging.basicConfig(format = '%(asctime)s %(message)s', filename = \
        'logs/Multiple_istgtcontrol_refresh_During_IO.log', filemode = 'a', \
        level = logging.DEBUG)
# Initialization for few common global variables

startTime = ctime()
HEADER_MSG = 'Testcase "Multiple_istgtcontrol_refresh_During_IO" is started'
FOOTER_MSG = 'Testcase "Multiple_istgtcontrol_refresh_During_IO" is completed'
BLOCKED_MSG = 'Testcase "Multiple_istgtcontrol_refresh_During_IO" is blocked'
EXECUTE_SYNATX = 'python Multiple_istgtcontrol_refresh_During_IO.py conf.txt'
...
def verify_list_controller(list_cntrl, startTime):
    if list_cntrl[0] == 'PASSED':
        return list_cntrl[1]
    logging.error('Testcase Multiple_istgtcontrol_refresh_During_IO is blocked due to' \
            ': %s',list_cntrl[1])
...
if vol_info[0] == 'FAILED':
    ...
    logging.error('Testcase Multiple_istgtcontrol_refresh_During_IO is '\
...
if discoverIqn[0] == 'FAILED':
    ...
    logging.debug('Testcase Multiple_istgtcontrol_refresh_During_IO is '\
...
if mountResult[0] == 'PASSED':
    CLIENT_NFS_MOUNT_PNT = mountResult[1]
    print 'volume is mounted successfully at %s' %(CLIENT_NFS_MOUNT_PNT)
else:
...
dicForVdbenchUse = {'name': VOL_NAME, 'mountPoint': CLIENT_NFS_MOUNT_PNT}

# Executing vdbench
logging.info('...executing vdbench....')
executeVdbenchFile(dicForVdbenchUse, 'filesystem_iscsi')
...
if iops > 0:
    ...
    logging.error('Testcase Multiple_istgtcontrol_refresh_During_IO is blocked due ' \
...
iops = get_iops_by_api(volId, STDURL)
if iops[0] == 'FAILED':
    logging.error('estcase Multiple_istgtcontrol_refresh_During_IO is blocked due to' \
            ': %s', iops[1])
    is_blocked(startTime, FOOTER_MSG, BLOCKED_MSG)
...
if iops > 0:
    ...
    logging.info('IOPS are running fine after multiple istgtcontrol refresh, '\
            'iops value is: %s', iops)
    logging.info('Testcase Multiple_istgtcontrol_refresh_During_IO PASSED...')
    resultCollection('Multiple_istgtcontrol_refresh_During_IO test case is:', \
    ...
    logging.info('Testcase Multiple_istgtcontrol_refresh_During_IO FAILED...')
    resultCollection('Multiple_istgtcontrol_refresh_During_IO test case is:', \
...
umountResult = executeCmd('umount %s' %(CLIENT_NFS_MOUNT_PNT))
...
if addIscsiAuthGrup[0] == 'FAILED':
    ...
    get_logger_footer('Multiple_istgtcontrol_refresh_During_IO test case is '\
            'completed')
    exit()
...
if delete_vol[0] == 'FAILED':
    ...
    get_logger_footer('Multiple_istgtcontrol_refresh_During_IO test case is completed')
    exit()
