'''
###############################################################################################################
# Testcase Name : Continuous_Metadata_Ops_NFS                                                                 #
#                                                                                                             #
# Testcase Description : This test performs file ops resulting in NFS verb storm :                            #
#                        a) Mounts and unmounts NFS volumes (FSINFO, PATHCONF)                                #
#                        b) Creates dirs & files with dd (MKDIR, CREATE, WRITE, ACCESS, LOOKUP, SETATTR)      #
#                        c) Creates soft links for each file (SYMLINK)                                        # 
#                        d) Retrieves fs info on each created file before read (FSTAT)                        #
#                        e) Reads and modifies files (ACCESS, GETATTR, READ, READLINK, RENAME, WRITE, SETATTR)#
#                        f) Lists dir contents after each fileOp (GETATTR, ACCESS, READDIRPLUS)               #
#                        g) Checks space consunption recursively after major operation chunks                 #
#                        h) Unlink files (UNLINK)                                                             #
#                        i) Delete files & dirs (REMOVE, RMDIR)                                               #
#                        j) Unmounts the NFS volume                                                           #                                                   
#                                                                                                             # 
# Testcase Pre-Requisites : At least one available node should be present                                     #
#                                                                                                             #
# Testcase Creation Date : 24/05/2016                                                                         #
#                                                                                                             #
# Testcase Last Modified : 24/05/2016                                                                         #  
#                                                                                                             #  
...
CLIENT_NFS_MOUNT_PNT = '/mnt/nfsmeta'
...
# Form the command to create the local mount point on the client 

mkdir_cmd = 'mkdir %s' %(CLIENT_NFS_MOUNT_PNT)

# Create the local mount point on client
...
src_file = 'AMSx-WLAx-NSWx-NFSx-xxxx-AUX1_METADATA_STRESS_OPS.py'
dst_file = src_file
...
run_workload_cmd = 'python %s %s %s %s' %(src_file, VSM_IP, vol_mnt_pt, CLIENT_NFS_MOUNT_PNT)

print run_workload_cmd
...
res = sshToOtherClient(CLIENT1_IP, CLIENT1_USER, CLIENT1_PASSWORD, run_workload_cmd)
...
test_result = sshToOtherClient(CLIENT1_IP, CLIENT1_USER, CLIENT1_PASSWORD, \
        'grep -w "CREATED\|LISTED\|CHECKED\|MODIFIED\|DELETED" NFSMetaDataStress.log | wc -l')
if int(test_result) == 8 :
