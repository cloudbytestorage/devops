...
def get_iops_by_api(vol_id, STDURL):
    querycommand = 'command=monitorFileSystemQoS&id=%s&type=s_access' %(vol_id)
    monitorFileSystemQoS_resp = sendrequest(STDURL, querycommand)
    monitorFileSystemQoS_resp = json.loads(monitorFileSystemQoS_resp.text)
    resp = str(monitorFileSystemQoS_resp)
    if 'monitorFileSystemResponse' in resp and 'monitorFilesystemQoS' in resp:
        iops = int(monitorFileSystemQoS_resp.get('monitorFileSystemResponse').\
                get('monitorFilesystemQoS')[-1].get('iops'))
        return ['PASSED', iops]
    else:
        print 'Not able to get monitor data for volume'
