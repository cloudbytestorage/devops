...
class WebUtils():
        ...
        try:
            ...
            while count1 < no_of_disks:
                ...
                try:
                    ...
                    self.log.error("Exceptio: Labling Disks", str(DA))
            self.driver.find_element_by_xpath(".//*[@class='cloud-button next final']").click()
            time.sleep(5)
            self.driver.implicitly_wait(300)
            message = self.driver.find_element_by_xpath(".//*[@class='message']/span/span").text
    ...
    def Create_DR(self, name, bkp_ip,):
        ...
        try:
            ...
            time.sleep(2)
            self.driver.find_element_by_xpath(".//*[@class='cloud-button next']").click()
            myelement = self.driver.find_element_by_xpath(".//*[@id='address']")
            if myelement.is_displayed():
                myelement.send_keys(bkp_ip)
            else:
                print ("BKP IP is already configured")
            self.driver.implicitly_wait(10)
...
def main():
    ...
    t = WebUtils()
    t.poweron("http://20.10.26.39/","ADMIN","ADMIN")
    time.sleep(5)
    t.login_EC(url,username,password)
    time.sleep(5)
    t.HA_Maintenance(1,"Maintenance")
    '''
